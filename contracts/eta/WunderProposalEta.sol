...
contract WunderProposalEta {
    ...
    function createProposal(
        ...
        require(bytes(_title).length > 0, "301: Missing Title");
        for (uint256 index = 0; index < _contractAddresses.length; index++) {
            require(
                _contractAddresses[index] != address(0),
                "302: Missing Address"
            );
            require(bytes(_actions[index]).length > 0, "303: Missing Action");
        }
    ...
    function vote(
        ...
        require(proposal.actions.length > 0, "305: Proposal does not exist");
        require(
        ...
        if (_mode == uint8(VoteType.Against)) {
            ...
            revert("308: Invalid VoteType (1=YES, 2=NO)");
        }
    }
