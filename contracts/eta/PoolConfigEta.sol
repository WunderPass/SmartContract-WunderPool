...
contract PoolConfigEta {
    ...
    function setupPool(
        ...
        require(!poolExists[_pool], "101: Pool already Exists");
        require(
            _votingThreshold <= 100,
            "102: Invalid Voting Threshold (0-100)"
        );
        require(_votingTime > 0, "103: Invalid Voting Time");
        require(_maxMembers > 0, "104: Invalid MaxMembers");
        require(_minYesVoters > 0, "105: Invalid minYesVoters");
        require(_minInvest > 0, "106: Invalid minInvest");
        require(
    ...
    function memberCanJoin(
        ...
        if (_amount < pools[_pool].minInvest) {
            return (false, "200: Stake is lower than minInvest");
        }
        if (_invested + _amount > pools[_pool].maxInvest) {
            return (false, "201: Stake is higher than maxInvest");
        }
        if (_members >= pools[_pool].maxMembers) {
            return (false, "202: Member Limit reached");
        }
        return (true, "");
    }
