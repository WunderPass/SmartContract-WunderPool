...
contract WunderSwapperBeta {
  ...
  function swapTokens(address _tokenIn, address _tokenOut, uint _amount) external {
    require(_amount > 0, "NOTHING TO TRADE");
    ...
    path[1] = _tokenOut;
    uint[] memory amounts = getAmounts(_amount, path);
    QuickSwapRouter(quickSwapRouterAddress).swapExactTokensForTokens(amounts[0], amounts[1], path, msg.sender, block.timestamp + 1200);
    emit SwappedTokens(msg.sender, _tokenIn, _tokenOut, amounts[0], amounts[1]);
  }
