...
const { usdc, topUp, approve, matic } = require('../helpers');
...
describe('WUNDER POOL CONTRACT', () => {
  ...
  beforeEach(async () => {
    ...
    poolLauncher = await (await ethers.getContractFactory('PoolLauncherGamma')).deploy();
    await topUp(owner, usdc(200));
    await approve(owner, poolLauncher.address, usdc(100));
    await poolLauncher.createNewPool("PublicApes", 0, "PublicApeToken", "PAT", usdc(0));
    await poolLauncher.createNewPool("PrivateApes", usdc(50), "PrivateApeToken", "PAT", usdc(100));
    const [free, paid] = await poolLauncher.allPools();
    freePool = await ethers.getContractAt("WunderPoolGamma", free, owner);
    paidPool = await ethers.getContractAt("WunderPoolGamma", paid, owner);
  });
  ...
  describe('Deployment', () => {
    it('Should Set the correct Parameters', async () => {
      ...
      expect(await paidPool.entryBarrier()).to.equal(usdc(50));
    });
  });
  ...
  describe('Roles', () => {
    it('Member cannot join twice', async () => {
      ...
      await expect(freePool.joinPool(0)).to.be.revertedWith("Already Member");
      expect((await freePool.poolMembers()).length).to.equal(1)
    });
    ...
    it('Member should be able to create Proposal', async () => {
      ...
      await freePool.connect(user1).joinPool(0);
  ...
  describe('Barrier', () => {
    it('Anyone can Join a Free Pool', async () => {
      await expect(freePool.connect(user1).joinPool(0)).to.not.be.reverted;
    ...
    it('Anyone can join a Paid Pool if the fee is high enough', async () => {
      await topUp(user1, usdc(51));
      await approve(user1, paidPool.address, usdc(50));
      await expect(paidPool.connect(user1).joinPool(0)).to.be.revertedWith("Your stake is not high enough");
      await expect(paidPool.connect(user1).joinPool(usdc(49))).to.be.revertedWith("Your stake is not high enough");
      await expect(paidPool.connect(user1).joinPool(usdc(50))).to.not.be.reverted;
  ...
  describe('Proposals', () => {
    it('Should revert for missing Parameters', async () => {
      await expect(paidPool.createProposal("", proposal.description, proposal.contractAddress, proposal.action, proposal.params, proposal.transactionValue, proposal.deadline)).to.be.revertedWith("Missing Title");
      await expect(paidPool.createProposal(proposal.title, proposal.description, nullAddress, proposal.action, proposal.params, proposal.transactionValue, proposal.deadline)).to.be.revertedWith("Missing Address");
      await expect(paidPool.createProposal(proposal.title, proposal.description, proposal.contractAddress, "", proposal.params, proposal.transactionValue, proposal.deadline)).to.be.revertedWith("Missing Action");
      await expect(paidPool.createProposal(proposal.title, proposal.description, proposal.contractAddress, proposal.action, proposal.params, proposal.transactionValue, 0)).to.be.revertedWith("Invalid Deadline");
    });

    it('Should revert for invalid Multi Action Parameters', async () => {
      await expect(paidPool.createMultiActionProposal(proposal.title, proposal.description, [proposal.contractAddress, proposal.contractAddress], [proposal.action], [proposal.params], [proposal.transactionValue], proposal.deadline)).to.be.revertedWith("Inconsistent amount of transactions");
      await expect(paidPool.createMultiActionProposal(proposal.title, proposal.description, [proposal.contractAddress], [proposal.action, proposal.action], [proposal.params], [proposal.transactionValue], proposal.deadline)).to.be.revertedWith("Inconsistent amount of transactions");
      await expect(paidPool.createMultiActionProposal(proposal.title, proposal.description, [proposal.contractAddress], [proposal.action], [proposal.params, proposal.params], [proposal.transactionValue], proposal.deadline)).to.be.revertedWith("Inconsistent amount of transactions");
      await expect(paidPool.createMultiActionProposal(proposal.title, proposal.description, [proposal.contractAddress], [proposal.action], [proposal.params], [proposal.transactionValue, proposal.transactionValue], proposal.deadline)).to.be.revertedWith("Inconsistent amount of transactions");
      await expect(paidPool.createMultiActionProposal(proposal.title, proposal.description, [proposal.contractAddress, proposal.contractAddress], [proposal.action, proposal.action], [proposal.params], [proposal.transactionValue], proposal.deadline)).to.be.revertedWith("Inconsistent amount of transactions");
    });
  ...
  describe('Voting', () => {
    let govToken;
    beforeEach(async () => {
      await topUp(user1, usdc(60));
      await approve(user1, paidPool.address, usdc(50));
      await paidPool.connect(user1).joinPool(usdc(50));
      await paidPool.createProposal(proposal.title, proposal.description, proposal.contractAddress, proposal.action, proposal.params, proposal.transactionValue, proposal.deadline);
    ...
    it('Voting should revert for Non Existing Proposal', async () => {
      await expect(paidPool.vote(1, 1)).to.be.revertedWith("Does not exist");
    });
    ...
    it('User should not be able to vote twice for the same Proposal', async () => {
      await paidPool.vote(0, 1);
      await expect(paidPool.vote(0, 1)).to.be.revertedWith("Already voted");
    });
    ...
    it('User should not be able to vote once proposal deadline is exceeded', async () => {
      ...
      await paidPool.vote(0, 1);
      await expect(paidPool.vote(1, 1)).to.be.revertedWith("Voting period has ended");
    });

    it('Voting should revert for Non Existing Voting Mode', async () => {
      await expect(paidPool.vote(0, 0)).to.be.revertedWith("Invalid VoteType (1=YES, 2=NO)");
      await expect(paidPool.vote(0, 3)).to.be.revertedWith("Invalid VoteType (1=YES, 2=NO)");
    });
  });
  ...
  describe('Execution', () => {
    ...
    beforeEach(async () => {
      ...
      await topUp(user1, usdc(60));
      await approve(user1, paidPool.address, usdc(50));
      await topUp(user2, usdc(80));
      await approve(user2, paidPool.address, usdc(70));
      
      await paidPool.connect(user1).joinPool(usdc(50));
      await paidPool.connect(user2).joinPool(usdc(70));
    ...
    it('Should revert for Non Existing Proposal', async () => {
      await expect(paidPool.executeProposal(1)).to.be.revertedWith("Does not exist");
    });

    it('Should revert if majority voted against it', async () => {
      await paidPool.connect(user1).vote(0, 2);
      await paidPool.connect(user2).vote(0, 2);
      await expect(paidPool.executeProposal(0)).to.be.revertedWith("Majority voted against execution");
    });

    it('Should revert if voting is still allowed and no majority is achieved', async () => {
      await expect(paidPool.executeProposal(0)).to.be.revertedWith("Voting still allowed");
    });
    ...
    it('Should revert if proposal transaction value is greater than pools balance', async () => {
      ...
      await expect(paidPool.executeProposal(1)).to.be.revertedWith("Not enough funds");
    });
    ...
    it('Should execute a correct call replacing msg.sender and passing msg.value to the function', async () => {
      await owner.sendTransaction({to: paidPool.address, value: matic(1)});
    ...
    it('Should not execute a proposal twice', async () => {
      ...
      await expect(paidPool.executeProposal(0)).to.be.revertedWith("Already executed");
    });
  });
});
